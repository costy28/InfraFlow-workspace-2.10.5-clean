...
const APP_VERSION = (() => {
  try { return require('../../../version.json').version }
  catch { try { return require('../../package.json').version } catch { return 'dev' } }
})()
...
function ensureWorkOrdersDb(db) {
  db.work = db.work || {}
  db.work.orders = Array.isArray(db.work.orders) ? db.work.orders : []
  db.work.order_departments = Array.isArray(db.work.order_departments) ? db.work.order_departments : []
  db.work.order_materials = Array.isArray(db.work.order_materials) ? db.work.order_materials : []
...
function workOrderView(db, user) {
  const work = ensureWorkOrdersDb(db)
  const dept = userDepartmentCod(db, user)
  let orders = work.orders
  if (!['superadmin', 'admin', 'technical'].includes(user.role)) {
    const orderIds = new Set(work.order_departments.filter(item => String(item.department_cod).toLowerCase() === dept).map(item => item.order_id))
    orders = orders.filter(order => orderIds.has(order.id))
  }
  return orders.map(order => ({
    ...order,
    departamente: work.order_departments.filter(item => item.order_id === order.id),
    materiale: work.order_materials.filter(item => item.order_id === order.id)
...
router.get('/work/orders', (req, res) => {
  ...
  if (!auth) return
  if (!requireAnyPermission(auth, res, ['technical:view', 'department_requests:view'])) return
  sendJson(res, 200, { orders: workOrderView(auth.db, auth.user) })
})
...
router.post('/work/orders', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res)
    if (!auth) return
    if (!requireAnyPermission(auth, res, ['technical:worklog', 'technical:view'])) return
    const body = await readJsonBody(req)
    const work = ensureWorkOrdersDb(auth.db)
    const order = { id: id('wo'), uuid: crypto.randomUUID(), titlu: body.titlu || '', descriere: body.descriere || '', tip: body.tip || 'lucrare', data_start: body.data_start || localDate(new Date()), data_termen: body.data_termen || '', status: 'in_lucru', creat_de: auth.user.id, created_at: new Date().toISOString() }
    if (!order.titlu) return sendJson(res, 400, { error: 'Titlul comenzii este obligatoriu.' })
    work.orders.push(order)
    ;(body.departamente || []).forEach(dept => work.order_departments.push({ id: id('wod'), order_id: order.id, department_cod: String(dept).toLowerCase(), status: 'primit', ore_estimate: 0, ore_efectuate: 0, observatii: '', finalizat_la: null }))
    addAudit(auth.db, auth.user, 'work_order_created', order.titlu)
    writeDb(auth.db)
...
router.patch('/work/orders/:uuid/department-status', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res)
    if (!auth) return
    if (!requireAnyPermission(auth, res, ['technical:worklog', 'department_requests:create'])) return
    const body = await readJsonBody(req)
    const work = ensureWorkOrdersDb(auth.db)
    const order = work.orders.find(item => item.uuid === req.params.uuid)
    if (!order) return sendJson(res, 404, { error: 'Comanda de lucru nu exista.' })
    const dept = String(body.department_cod || userDepartmentCod(auth.db, auth.user)).toLowerCase()
    let row = work.order_departments.find(item => item.order_id === order.id && String(item.department_cod).toLowerCase() === dept)
    if (!row) {
      row = { id: id('wod'), order_id: order.id, department_cod: dept, status: 'primit', ore_estimate: 0, ore_efectuate: 0, observatii: '', finalizat_la: null }
      work.order_departments.push(row)
    }
    row.status = body.status || row.status
    row.ore_efectuate = Number(body.ore_efectuate ?? row.ore_efectuate ?? 0)
    row.observatii = body.observatii ?? row.observatii
    if (row.status === 'finalizat') row.finalizat_la = new Date().toISOString()
    if (work.order_departments.filter(item => item.order_id === order.id).every(item => item.status === 'finalizat')) order.status = 'finalizat'
    addAudit(auth.db, auth.user, 'work_order_department_status', `${order.titlu} / ${dept} / ${row.status}`)
    writeDb(auth.db)
    sendJson(res, 200, { order, department: row })
  } catch (error) {
    next(error)
  }
})

router.post('/work/orders/:uuid/consume-material', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res)
    if (!auth) return
    if (!requireAnyPermission(auth, res, ['technical:worklog', 'stock_operations:create', 'inventory:exits'])) return
    const body = await readJsonBody(req)
    const work = ensureWorkOrdersDb(auth.db)
    const order = work.orders.find(item => item.uuid === req.params.uuid)
    if (!order) return sendJson(res, 404, { error: 'Comanda de lucru nu exista.' })
    ...
    const stock = auth.db.inventory.department_stocks.find(item => String(item.material_id) === String(body.material_id || body.materialId) && String(item.department_cod).toLowerCase() === dept)
    if (!stock || Number(stock.cantitate || 0) < amount) return sendJson(res, 409, { error: 'Stoc departament insuficient.' })
    ...
    const row = { id: id('wom'), order_id: order.id, department_cod: dept, material_id: body.material_id || body.materialId, materialName: material?.name || material?.denumire || '', cantitate: amount, created_at: new Date().toISOString() }
    work.order_materials.push(row)
    addAudit(auth.db, auth.user, 'work_order_material_consumed', `${order.titlu} / ${row.materialName} / ${fmt(amount)}`)
    writeDb(auth.db)
    sendJson(res, 201, { material: row, stock })
  } catch (error) {
    next(error)
  }
})

router.post('/work/orders/month-close', (req, res) => {
  const auth = requireAuth(req, res)
  if (!auth) return
  if (!requirePermission(auth, res, 'technical:view')) return
  const work = ensureWorkOrdersDb(auth.db)
  let count = 0
  work.orders.forEach(order => {
    if (order.status === 'in_lucru') {
      order.status = 'partial'
      count += 1
    }
  })
  addAudit(auth.db, auth.user, 'work_orders_month_close', `${count} comenzi marcate partial`)
  writeDb(auth.db)
  sendJson(res, 200, { ok: true, partial: count })
})

router.get('/technical/work-logs', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
...
router.post('/technical/work-logs', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
...
router.get('/technical/asphalt-sales', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
  if (!requirePermission(auth, res, "technical:view")) return;
...
router.post('/technical/asphalt-sales', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
...
router.get('/technical/clients', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  if (!requirePermission(auth, res, "technical:view")) return;
...
router.post('/technical/clients', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    if (!requirePermission(auth, res, "technical:sales")) return;
...
router.get('/technical/clients/lookup-cif', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    const url = makeUrl(req);
    if (!requirePermission(auth, res, "technical:sales")) return;
    const client = await lookupAnafClient(url.searchParams.get("cif"));
...
router.post('/technical/clients/lookup-cif', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    if (!requirePermission(auth, res, "technical:sales")) return;
...
router.get('/technical/report', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
  if (!requirePermission(auth, res, "technical:view")) return;
...
router.get('/accounting/asphalt-sales', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/nexus-expenses', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.post('/nexus-expenses', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    if (!requirePermission(auth, res, "cost_accounting:import")) return;
    const body = await readJsonBody(req, 10_000_000);
    const result = importNexusExpenses(auth.db, auth.user, { rows: Array.isArray(body.rows) ? body.rows : [body] });
...
router.post('/nexus-expenses/import', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    if (!requirePermission(auth, res, "cost_accounting:import")) return;
...
router.post('/nexus-expenses/import-xlsx', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    if (!requirePermission(auth, res, "cost_accounting:import")) return;
...
router.get('/cost-accounting/report', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
  if (!requirePermission(auth, res, "cost_accounting:view")) return;
...
router.get('/exports/technical-report.xlsx', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/exports/cost-accounting.xlsx', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/reports/technical-report', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
  if (!requirePermission(auth, res, "technical:export")) return;
...
router.get('/reports/cost-accounting', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
  if (!requirePermission(auth, res, "cost_accounting:export")) return;
