...
const router = Router();
const TYPES = new Set(["bonus", "beneficiu_impozabil", "retinere", "indemnizatie_medicala"]);
...
router.patch("/hr/payroll/adjustments/:id", salaryManage, (req, res, next) => {
  try {
    ...
    ["tip", "cod", "descriere", "data_start", "data_sfarsit", "recurent", "active"].forEach((key) => {
...
router.get("/hr/payroll/:runId/bank-export", salaryView, (req, res, next) => {
  try {
    ...
    const sheet = xlsx.utils.aoa_to_sheet([["Plati salarii", run.luna, `Total ${run.total_net} RON`], [], ["Nr.", "Beneficiar", "IBAN", "Suma RON", "Explicatie"], ...rows]);
...
router.post("/hr/payroll/:runId/post-accounting", salaryManage, (req, res, next) => {
  try {
    ...
    if (run.accounting_journal_id) throwHttp(409, "Statul salarial are deja nota contabila generata.");
    ...
    const lines = [
      ...
      ...(run.total_other_deductions > 0 ? [{ cont: "427", credit: run.total_other_deductions, explicatie: `Retineri salarii ${run.luna}` }] : []),
