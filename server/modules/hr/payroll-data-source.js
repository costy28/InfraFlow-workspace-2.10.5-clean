...
function hydratePayrollInputs(hr) {
  if (!MSSQL_RELATIONAL_MODE || !["mssql", "sqlserver"].includes(DB_MODE)) return hr;
  ...
  hr.contracts = readRows(`SELECT * FROM hr.contracts WHERE ISNULL(status,N'activ')<>N'incetat' FOR JSON PATH;`);
