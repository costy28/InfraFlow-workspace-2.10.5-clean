...
router.post('/consumptions', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
...
router.get('/config/recipes-materials', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
...
router.post('/config/recipes-materials', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    if (!requirePermission(auth, res, "settings:manage")) return;
...
router.get('/department-consumptions', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.post('/department-consumption', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
...
router.get('/exports/consumptions.xlsx', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/exports/accounting-report.xlsx', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/reports/consumptions', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
  if (!requirePermission(auth, res, "consumptions:export")) return;
...
router.get('/reports/accounting-report', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
// ─── RAPORT ZILNIC PRODUCȚIE ──────────────────────────────────────────────────
// GET /api/production/raport-zilnic?luna=2026-05  → agregat pe zile
// GET /api/production/raport-zilnic?data=2026-05-26  → detaliu o zi
router.get('/production/raport-zilnic', (req, res, next) => {
  try {
    const auth = requireAuth(req, res)
    if (!auth) return
    if (!requireAnyPermission(auth, res, ['consumptions:view', 'consumptions:export'])) return

    const db = auth.db
    const { luna, data } = req.query
    const consumptions = Array.isArray(db.consumptions) ? db.consumptions : []
    ...
    const filtered = consumptions.filter(c => {
      const d = c.date || c.data || ''
      if (luna) return d.startsWith(luna)
      if (data) return d === data
      return true
    })

    // Helper: compute material consumption for a batch
    function materialBreakdown(consumption) {
      const recipe = recipes.find(r => r.id === (consumption.recipeId || consumption.retetaId))
      if (!recipe) return []
      const tons = Number(consumption.asphalt || consumption.tone || 0)
      const percentages = recipe.percentages || recipe.components || recipe.componente || {}
      if (Array.isArray(percentages)) {
        return percentages.map(p => {
          const mat = materials.find(m => String(m.id) === String(p.materialId || p.id))
          const qty = tons * Number(p.percent || 0) / 100
          return { materialId: p.materialId || p.id, materialName: mat?.name || mat?.denumire || p.materialId, unit: mat?.unit || mat?.um || 't', qty: Math.round(qty * 1000) / 1000 }
        }).filter(x => x.qty > 0)
      }
      return Object.entries(percentages).map(([matId, pct]) => {
        const mat = materials.find(m => String(m.id) === String(matId))
        const qty = tons * Number(pct) / 100
        return { materialId: matId, materialName: mat?.name || mat?.denumire || matId, unit: mat?.unit || mat?.um || 't', qty: Math.round(qty * 1000) / 1000 }
      }).filter(x => x.qty > 0)
    }
    ...
    if (data) {
      ...
      const rows = filtered.map(c => ({
        ...c,
        recipeName: (recipes.find(r => r.id === (c.recipeId || c.retetaId)) || {}).name || c.recipeName || '',
        materiale: materialBreakdown(c),
      }))
      const totalTone = rows.reduce((s, c) => s + Number(c.asphalt || c.tone || 0), 0)
      // Aggregate material totals for the day
      const matTotals = {}
      rows.forEach(c => c.materiale.forEach(m => {
        if (!matTotals[m.materialId]) matTotals[m.materialId] = { materialId: m.materialId, materialName: m.materialName, unit: m.unit, qty: 0 }
        matTotals[m.materialId].qty += m.qty
      }))
      sendJson(res, 200, { data, rows, totalTone: Math.round(totalTone * 1000) / 1000, materialeTotale: Object.values(matTotals) })
      return
    }
    ...
    const byDay = {}
    filtered.forEach(c => {
      const d = (c.date || c.data || '').slice(0, 10)
      if (!d) return
      if (!byDay[d]) byDay[d] = { data: d, productii: 0, tone_total: 0, retete: new Set(), materiale: {} }
      const day = byDay[d]
      day.productii += 1
      const tone = Number(c.asphalt || c.tone || 0)
      day.tone_total += tone
      const recipe = recipes.find(r => r.id === (c.recipeId || c.retetaId))
      if (recipe) day.retete.add(recipe.name || recipe.id)
      materialBreakdown(c).forEach(m => {
        if (!day.materiale[m.materialId]) day.materiale[m.materialId] = { materialName: m.materialName, unit: m.unit, qty: 0 }
        day.materiale[m.materialId].qty += m.qty
      })
    })

    const zile = Object.values(byDay)
      .map(d => ({
        data: d.data,
        productii: d.productii,
        tone_total: Math.round(d.tone_total * 1000) / 1000,
        retete: [...d.retete].join(', '),
        materiale: Object.values(d.materiale).map(m => ({ ...m, qty: Math.round(m.qty * 1000) / 1000 })),
      }))
      .sort((a, b) => b.data.localeCompare(a.data))

    const totals = {
      tone_total: Math.round(zile.reduce((s, z) => s + z.tone_total, 0) * 1000) / 1000,
      zile_productie: zile.length,
      productii_total: zile.reduce((s, z) => s + z.productii, 0),
    }

    sendJson(res, 200, { luna: luna || null, zile, totals })
  } catch (err) { next(err) }
})

// ─── LINK PRODUCȚIE → GESTIUNE (scăzut stoc la confirmare consum) ─────────────
router.post('/production/consumptions/link-gestiune/:id', (req, res, next) => {
  try {
    const auth = requireAuth(req, res)
    if (!auth) return
    if (!requireAnyPermission(auth, res, ['consumptions:create', 'stock_operations:create'])) return

    const db = readDb()
    const consumption = (db.consumptions || []).find(c => String(c.id) === req.params.id)
    if (!consumption) return sendJson(res, 404, { error: 'Consumul nu a fost găsit.' })
    if (consumption.gestiune_linked) return sendJson(res, 409, { error: 'Consumul este deja legat de Gestiune.' })

    const recipes = db.recipes || []
    const recipe = recipes.find(r => r.id === (consumption.recipeId || consumption.retetaId))
    if (!recipe) return sendJson(res, 404, { error: 'Rețeta nu a fost găsită.' })

    const tons = Number(consumption.asphalt || consumption.tone || 0)
    const percentages = recipe.percentages || recipe.components || recipe.componente || {}
    const errors = []

    const entries = Array.isArray(percentages)
      ? percentages.map(p => ({ matId: String(p.materialId || p.id), pct: Number(p.percent || 0) }))
      : Object.entries(percentages).map(([k, v]) => ({ matId: String(k), pct: Number(v) }))

    entries.forEach(({ matId, pct }) => {
      const qty = Math.round(tons * pct / 100 * 1000) / 1000
      if (qty <= 0) return
      const mat = (db.materials || []).find(m => String(m.id) === matId)
      if (!mat) return
      const stoc = Number(mat.stock ?? mat.stoc_curent ?? mat.currentStock ?? 0)
      if (stoc < qty) errors.push(`Stoc insuficient: ${mat.name || mat.denumire} (disponibil ${stoc}, necesar ${qty})`)
    })

    if (errors.length) return sendJson(res, 409, { error: errors.join('; ') })
    ...
    entries.forEach(({ matId, pct }) => {
      ...
      if (!mat) return
      const stoc = Number(mat.stock ?? mat.stoc_curent ?? mat.currentStock ?? 0)
      mat.stock = Math.max(0, stoc - qty)
      mat.stoc_curent = mat.stock
      mat.currentStock = mat.stock
      mat.updated_at = new Date().toISOString()
      // stock movement
      if (!Array.isArray(db.stockMovements)) db.stockMovements = []
      db.stockMovements.push({
        id: `mov-${Date.now()}-${matId}`,
        date: consumption.date || consumption.data || new Date().toISOString().slice(0, 10),
        materialId: mat.id, materialName: mat.name || mat.denumire,
        amount: -qty, unit: mat.unit || mat.um || 't',
        type: 'productie', reason: `Consum producție: ${recipe.name || recipe.id} / ${tons}t`,
        docNr: String(consumption.id), createdBy: auth.user?.id || 'system', createdAt: new Date().toISOString(),
      })
    })

    consumption.gestiune_linked = true
    consumption.gestiune_linked_at = new Date().toISOString()
    consumption.gestiune_linked_by = auth.user?.name || ''
    addAudit(db, auth.user, 'productie_legata_gestiune', `Consum ${consumption.id} / ${tons}t`)
    writeDb(db)
    sendJson(res, 200, { ok: true, consumption })
  } catch (err) { next(err) }
})
