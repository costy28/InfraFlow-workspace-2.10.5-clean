...
function ensureInventoryExtensions(db) {
  db.inventory = db.inventory || {}
  db.inventory.department_stocks = Array.isArray(db.inventory.department_stocks) ? db.inventory.department_stocks : []
  db.inventory.stock_transfers = Array.isArray(db.inventory.stock_transfers) ? db.inventory.stock_transfers : []
  db.inventory.department_consumptions = Array.isArray(db.inventory.department_consumptions) ? db.inventory.department_consumptions : []
  return db.inventory
}
...
function centralStock(material) {
  return Number(material?.stock ?? material?.stoc_curent ?? material?.currentStock ?? 0)
}

function setCentralStock(material, value) {
  material.stock = Number(value || 0)
  material.stoc_curent = Number(value || 0)
}

function userDepartmentCod(db, user) {
  if (user?.department_cod) return String(user.department_cod).toLowerCase()
  if (user?.department) return String(user.department).toLowerCase()
  const dept = (db.departments || []).find(item => String(item.id) === String(user?.departmentId || user?.department_id))
  return String(dept?.cod || dept?.code || dept?.denumire || dept?.name || '').toLowerCase()
}

function isInventoryManager(auth) {
  return ['superadmin', 'admin', 'inventory'].includes(auth.user.role)
}

function departmentStockRow(db, materialId, departmentCod) {
  const inv = ensureInventoryExtensions(db)
  let row = inv.department_stocks.find(item => String(item.material_id) === String(materialId) && String(item.department_cod).toLowerCase() === String(departmentCod).toLowerCase())
  if (!row) {
    row = { id: id('dept-stock'), material_id: materialId, department_cod: String(departmentCod).toLowerCase(), cantitate: 0, ultima_actualizare: new Date().toISOString() }
    inv.department_stocks.push(row)
  }
  return row
}

function departmentStockView(db, departmentCod) {
  const inv = ensureInventoryExtensions(db)
  return inv.department_stocks
    .filter(item => !departmentCod || String(item.department_cod).toLowerCase() === String(departmentCod).toLowerCase())
    .map(item => {
      const material = (db.materials || []).find(mat => String(mat.id) === String(item.material_id))
      return {
        ...item,
        materialName: materialLabel(material),
        unit: materialUnit(material),
        stoc_curent: Number(item.cantitate || 0),
        stoc_minim: Number(material?.alert || material?.stoc_minim || material?.stockMin || 0)
      }
    })
}

function centralStockView(db) {
  return (db.materials || []).map(material => ({
    id: material.id,
    material_id: material.id,
    department_cod: 'central',
    materialName: materialLabel(material),
    name: materialLabel(material),
    unit: materialUnit(material),
    stoc_curent: centralStock(material),
    stock: centralStock(material),
    stoc_minim: Number(material.alert || material.stoc_minim || material.stockMin || 0)
  }))
}
...
function addStockMovement(db, payload) {
  ...
  db.stockMovements.push({
    id: id('mov'),
    date: payload.date || localDate(new Date()),
...
router.get('/inventory/my-stock', (req, res) => {
  const auth = requireAuth(req, res)
  if (!auth) return
  if (!requireAnyPermission(auth, res, ['materials:view', 'inventory:view', 'stock_operations:view'])) return
  if (isInventoryManager(auth)) return sendJson(res, 200, { department: 'central', stocks: centralStockView(auth.db) })
  const dept = userDepartmentCod(auth.db, auth.user)
  sendJson(res, 200, { department: dept, stocks: departmentStockView(auth.db, dept) })
})

router.get('/inventory/department-stock/:dept', (req, res) => {
  const auth = requireAuth(req, res)
  if (!auth) return
  if (!requireAnyPermission(auth, res, ['materials:view', 'inventory:view', 'stock_operations:view'])) return
  sendJson(res, 200, { department: req.params.dept, stocks: departmentStockView(auth.db, req.params.dept) })
})

router.get('/inventory/transfers', (req, res) => {
  const auth = requireAuth(req, res)
  if (!auth) return
  if (!requireAnyPermission(auth, res, ['materials:view', 'inventory:view', 'stock_operations:view'])) return
  const inv = ensureInventoryExtensions(auth.db)
  let transfers = inv.stock_transfers.slice().sort((a, b) => String(b.created_at || '').localeCompare(String(a.created_at || '')))
  if (!isInventoryManager(auth)) {
    const dept = userDepartmentCod(auth.db, auth.user)
    transfers = transfers.filter(item => String(item.to_department).toLowerCase() === dept || String(item.from_department || '').toLowerCase() === dept)
  }
...
router.post('/inventory/transfers', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res)
    if (!auth) return
    if (!requireAnyPermission(auth, res, ['stock_operations:create', 'inventory:transfers', 'inventory:exits'])) return
    const body = await readJsonBody(req)
    const material = (auth.db.materials || []).find(item => String(item.id) === String(body.material_id || body.materialId))
    if (!material) return sendJson(res, 404, { error: 'Material inexistent.' })
    const amount = Number(body.cantitate || body.amount || 0)
    if (amount <= 0) return sendJson(res, 400, { error: 'Cantitatea trebuie sa fie pozitiva.' })
    if (centralStock(material) < amount) return sendJson(res, 409, { error: 'Stoc central insuficient.' })
    const toDept = String(body.to_department || body.department || '').trim().toLowerCase()
    if (!toDept) return sendJson(res, 400, { error: 'Departamentul destinatar este obligatoriu.' })
    setCentralStock(material, centralStock(material) - amount)
    const row = departmentStockRow(auth.db, material.id, toDept)
    row.cantitate = Number(row.cantitate || 0) + amount
    row.ultima_actualizare = new Date().toISOString()
    const inv = ensureInventoryExtensions(auth.db)
    const transfer = {
      id: id('trf'),
    ...
    inv.stock_transfers.push(transfer)
    addStockMovement(auth.db, { materialId: material.id, materialName: materialLabel(material), amount: -amount, unit: materialUnit(material), type: 'transfer_department', reason: `Transfer catre ${toDept}`, createdBy: auth.user.id })
    addAudit(auth.db, auth.user, 'inventory_transfer', `${materialLabel(material)} -> ${toDept} / ${fmt(amount)} ${materialUnit(material)}`)
    writeDb(auth.db)
    sendJson(res, 201, { transfer, stock: row })
  } catch (error) {
    next(error)
  }
})

router.post('/inventory/transfers/:uuid/approve', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res)
    if (!auth) return
    if (!requireAnyPermission(auth, res, ['stock_operations:create', 'inventory:transfers'])) return
    const inv = ensureInventoryExtensions(auth.db)
    const transfer = inv.stock_transfers.find(item => item.uuid === req.params.uuid)
    if (!transfer) return sendJson(res, 404, { error: 'Transfer inexistent.' })
    transfer.status = 'aprobat'
    transfer.aprobat_de = auth.user.id
    transfer.aprobat_la = new Date().toISOString()
    addAudit(auth.db, auth.user, 'inventory_transfer_approved', transfer.uuid)
    writeDb(auth.db)
...
router.post('/inventory/department-consumption', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res)
    if (!auth) return
    if (!requireAnyPermission(auth, res, ['inventory:exits', 'stock_operations:create', 'department_requests:create'])) return
    const body = await readJsonBody(req)
    const dept = String(body.department_cod || userDepartmentCod(auth.db, auth.user)).toLowerCase()
    const material = (auth.db.materials || []).find(item => String(item.id) === String(body.material_id || body.materialId))
    if (!material) return sendJson(res, 404, { error: 'Material inexistent.' })
    const amount = Number(body.cantitate || body.amount || 0)
    const stock = departmentStockRow(auth.db, material.id, dept)
    if (Number(stock.cantitate || 0) < amount) return sendJson(res, 409, { error: 'Stoc departament insuficient.' })
    stock.cantitate = Number(stock.cantitate || 0) - amount
    stock.ultima_actualizare = new Date().toISOString()
    const inv = ensureInventoryExtensions(auth.db)
    const consumption = { id: id('dept-cons'), uuid: crypto.randomUUID(), material_id: material.id, materialName: materialLabel(material), department_cod: dept, cantitate: amount, lucrare_id: body.lucrare_id || body.work_order_id || '', motiv: body.motiv || '', creat_de: auth.user.id, created_at: new Date().toISOString() }
    inv.department_consumptions.push(consumption)
    addAudit(auth.db, auth.user, 'inventory_department_consumption', `${dept}: ${materialLabel(material)} / ${fmt(amount)} ${materialUnit(material)}`)
    writeDb(auth.db)
    sendJson(res, 201, { consumption, stock })
  } catch (error) {
    next(error)
  }
})
...
router.get('/reports/daily-report', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/period-report', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/reports/period-report', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.post('/initial-stock/complete', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    ...
    writeDb(auth.db);
    sendJson(res, 200, { settings, dashboard: buildDashboard(auth.db) });
  } catch (error) {
    next(error);
  }
})
...
router.get('/material-options', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
...
router.post('/stock-operations', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
...
router.get('/stock-operations', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/department-stocks', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
...
router.post('/department-transfers', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    if (!requirePermission(auth, res, "stock_operations:create")) return;
...
router.get('/exports/daily-report.xlsx', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/exports/period-report.xlsx', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/exports/stock-operations.xlsx', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
...
router.get('/exports/ledger.xlsx', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  const url = makeUrl(req);
