...
router.get('/fleet-assets', (req, res) => {
...
router.post('/fleet-requests', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
...
router.get('/fleet-assets/import-preview', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    if (!requirePermission(auth, res, "mechanization:manage")) return;
...
router.post('/fleet-assets/import-preview', async (req, res, next) => {
  try {
    const auth = requireAuth(req, res);
    if (!auth) return;
    if (!requirePermission(auth, res, "mechanization:manage")) return;
...
router.get('/exports/fleet.xlsx', (req, res) => {
  const auth = requireAuth(req, res);
  if (!auth) return;
  if (!requirePermission(auth, res, "mechanization:view")) return;
...
async function handleApi(req, res, url) {
  ...
  if (req.method === "GET" && url.pathname === "/api/fleet-assets") {
    if (!requirePermission(auth, res, "mechanization:view")) return;
    sendJson(res, 200, { assets: fleetAssetsView(auth.db, Object.fromEntries(url.searchParams.entries())) });
    return;
  }
...
function normalizeFleetAssetKind(asset = {}) {
  const explicit = String(asset.tip_asset || asset.tipAsset || "").trim().toLowerCase();
  ...
  const registration = String(asset.nr_inmatriculare || asset.registration || "").trim().toUpperCase();
...
function fleetAssetSearchText(asset = {}) {
  return [
...
function fleetAssetDepartmentValue(asset = {}) {
  return String(asset.departament || asset.department || asset.departmentName || asset.locatie || asset.location || "").trim();
}

function fleetAssetsView(db, filters = {}) {
  const tip = String(filters.tip || filters.tip_asset || "").trim().toLowerCase();
  const search = String(filters.search || "").trim().toLowerCase();
  const status = String(filters.status || "").trim().toLowerCase();
  const departament = String(filters.departament || filters.department || "").trim().toLowerCase();
  return (db.fleetAssets || [])
    .map((asset) => ({
      ...asset,
      tip_asset: normalizeFleetAssetKind(asset),
      nr_inmatriculare: asset.nr_inmatriculare || asset.registration || "",
      cod: asset.cod || asset.assetCode || asset.inventoryNo || asset.registration || "",
      marca: asset.marca || asset.brand || "",
      departament: asset.departament || asset.department || ""
    }))
    .filter((asset) => {
      if (tip && asset.tip_asset !== tip) return false;
      if (search && !fleetAssetSearchText(asset).includes(search)) return false;
      if (status && status !== "toate" && String(asset.status || "").trim().toLowerCase() !== status) return false;
      if (departament && departament !== "toate" && fleetAssetDepartmentValue(asset).toLowerCase() !== departament) return false;
      return true;
    })
    .sort((a, b) => String(a.name || a.registration || a.cod || "").localeCompare(String(b.name || b.registration || b.cod || "")));
}
...
function createFleetAsset(db, user, body) {
  ...
  const name = String(body.name || "").trim();
  const registration = String(body.registration || body.nr_inmatriculare || body.cod || body.assetCode || "").trim().toUpperCase();
  ...
  const asset = {
    ...
    tip_asset: tipAsset,
    registration,
    nr_inmatriculare: tipAsset === "autovehicul" ? registration : String(body.nr_inmatriculare || "").trim().toUpperCase(),
    cod: String(body.cod || body.assetCode || body.inventoryNo || registration).trim(),
    name: name || registration,
    type: String(body.type || "").trim(),
    brand: String(body.brand || "").trim(),
    marca: String(body.marca || body.brand || "").trim(),
    ...
    tip_combustibil: String(body.tip_combustibil || body.fuelType || "").trim(),
    tankCapacity: fleetNumber(body.tankCapacity),
    standardConsumption: fleetNumber(body.standardConsumption),
    consum_normat_km: fleetNumber(body.consum_normat_km || body.consumNormatKm || body.standardConsumption),
    consum_orar_normat: fleetNumber(body.consum_orar_normat || body.consumOrarNormat || body.standardConsumptionHour),
    gps_device_id: String(body.gps_device_id || body.gpsDeviceId || "").trim(),
    sofer_principal_id: body.sofer_principal_id || body.driverId || body.sofer_id || null,
    meterUnit,
    initialMeter,
    currentMeter,
    lastMeterDate,
