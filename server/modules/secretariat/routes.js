...
function ensureDb(db){db.secretariat=db.secretariat||{}; for(const k of ['registry','correspondenceTracking','appointments']) db.secretariat[k]=Array.isArray(db.secretariat[k])?db.secretariat[k]:[]; return db.secretariat}
function deptName(db,id){return (db.departments||db.core?.departments||[]).find(d=>String(d.id)===String(id))?.denumire||''}
function userName(db,id){const u=(db.users||db.core?.users||[]).find(x=>String(x.id)===String(id)); return [u?.prenume,u?.nume].filter(Boolean).join(' ')||u?.name||''}
function findDepartmentId(db,value){const raw=String(value||'').trim(); if(!raw)return ''; const normalized=raw.toLowerCase(); const departments=db.departments||db.core?.departments||[]; const found=departments.find(d=>String(d.id)===raw)||departments.find(d=>String(d.cod||'').toLowerCase()===normalized)||departments.find(d=>String(d.denumire||d.name||'').toLowerCase()===normalized); return found?.id||raw}
function resolveRegistryAssignment(db,body){const deptId=findDepartmentId(db,body.dept_destinatar||body.departament||body.department||body.dept_id); const userId=body.user_responsabil||body.repartizat_catre||body.user_id||''; return {...body,dept_destinatar:deptId,user_responsabil:userId}}
...
router.post('/secretariat/registry/next-number',(req,res,next)=>{try{
  const auth=requireAuth(req,res); if(!auth)return; if(!requirePermission(auth,res,'secretariat:view'))return
  if(isMssqlMode())return sendJson(res,200,mssqlObject(`DECLARE @year int=YEAR(GETDATE()); DECLARE @tip nvarchar(40)=COALESCE(NULLIF(JSON_VALUE(@p,'$.tip'),''),N'intrare_externa'); DECLARE @prefix nvarchar(5)=CASE WHEN @tip=N'iesire_externa' THEN N'E' WHEN @tip IN (N'intrare_interna',N'iesire_interna') THEN N'INT' ELSE N'I' END; DECLARE @nr int=COALESCE((SELECT MAX(nr_curent)+1 FROM secretariat.registry WHERE an=@year AND nr_inregistrare LIKE @prefix+N'-%'),1); SELECT @nr AS nr_curent, CONCAT(@prefix,N'-',RIGHT(CONCAT(N'000',@nr),3),N'/',@year) AS format, @prefix AS prefix FOR JSON PATH;`,req.body||{}))
...
router.post('/secretariat/registry/:id/assign',(req,res,next)=>{try{
  const auth=requireAuth(req,res); if(!auth)return; if(!requirePermission(auth,res,'secretariat:assign'))return
  const db=readDb()
  if(isMssqlMode()){
...
router.get('/secretariat/appointments',(req,res,next)=>{try{const auth=requireAuth(req,res); if(!auth)return; if(!requirePermission(auth,res,'secretariat:view'))return; if(isMssqlMode())return sendJson(res,200,mssqlArray(`SELECT * FROM secretariat.appointments WHERE (NULLIF(JSON_VALUE(@p,'$.tip'),'') IS NULL OR tip=JSON_VALUE(@p,'$.tip')) AND (NULLIF(JSON_VALUE(@p,'$.de_la'),'') IS NULL OR data_start>=TRY_CONVERT(datetime2,JSON_VALUE(@p,'$.de_la'))) AND (NULLIF(JSON_VALUE(@p,'$.pana_la'),'') IS NULL OR data_start<DATEADD(day,1,TRY_CONVERT(datetime2,JSON_VALUE(@p,'$.pana_la')))) ORDER BY data_start ASC FOR JSON PATH;`,req.query)); let rows=ensureDb(readDb()).appointments; if(req.query.tip)rows=rows.filter(x=>x.tip===req.query.tip); if(req.query.de_la)rows=rows.filter(x=>x.data_start>=req.query.de_la); if(req.query.pana_la)rows=rows.filter(x=>x.data_start<=req.query.pana_la); sendJson(res,200,rows)}catch(e){next(e)}})
router.post('/secretariat/appointments',(req,res,next)=>{try{const auth=requireAuth(req,res); if(!auth)return; if(!requirePermission(auth,res,'secretariat:appointments'))return; const db=readDb(); const body={...req.body,uuid:crypto.randomUUID(),participanti_json:JSON.stringify(req.body.participanti||[])}; if(isMssqlMode()){const item=mssqlObject(`INSERT INTO secretariat.appointments (uuid,tip,titlu,data_start,data_sfarsit,participanti_json,status) VALUES (JSON_VALUE(@p,'$.uuid'),NULLIF(JSON_VALUE(@p,'$.tip'),''),JSON_VALUE(@p,'$.titlu'),TRY_CONVERT(datetime2,JSON_VALUE(@p,'$.data_start')),TRY_CONVERT(datetime2,NULLIF(JSON_VALUE(@p,'$.data_sfarsit'),'')),JSON_VALUE(@p,'$.participanti_json'),COALESCE(NULLIF(JSON_VALUE(@p,'$.status'),''),N'programat')); SELECT TOP 1 * FROM secretariat.appointments WHERE id=SCOPE_IDENTITY() FOR JSON PATH;`,body); addAudit(db,auth.user,'secretariat_appointment_created',item?.uuid); writeDb(db); return sendJson(res,201,item)} const sec=ensureDb(db); const item={id:nextId(sec.appointments),...body,status:body.status||'programat',created_at:nowIso(),updated_at:null}; sec.appointments.push(item); addAudit(db,auth.user,'secretariat_appointment_created',item.uuid); writeDb(db); sendJson(res,201,item)}catch(e){next(e)}})
router.patch('/secretariat/appointments/:uuid/minuta',(req,res,next)=>{try{const auth=requireAuth(req,res); if(!auth)return; if(!requirePermission(auth,res,'secretariat:appointments'))return; const db=readDb(); if(isMssqlMode()){const item=mssqlObject(`UPDATE secretariat.appointments SET minuta=JSON_VALUE(@p,'$.minuta'), status=N'finalizat', updated_at=sysdatetime() WHERE uuid=JSON_VALUE(@p,'$.uuid'); SELECT TOP 1 * FROM secretariat.appointments WHERE uuid=JSON_VALUE(@p,'$.uuid') FOR JSON PATH;`,{...req.body,uuid:req.params.uuid}); addAudit(db,auth.user,'secretariat_appointment_minutes',req.params.uuid); writeDb(db); return sendJson(res,200,item)} const sec=ensureDb(db); const item=sec.appointments.find(x=>x.uuid===req.params.uuid); if(!item)return sendJson(res,404,{error:'Programarea nu a fost găsită.'}); item.minuta=req.body.minuta; item.status='finalizat'; item.updated_at=nowIso(); addAudit(db,auth.user,'secretariat_appointment_minutes',item.uuid); writeDb(db); sendJson(res,200,item)}catch(e){next(e)}})
router.get('/secretariat/overdue',(req,res,next)=>{try{const auth=requireAuth(req,res); if(!auth)return; if(!requirePermission(auth,res,'secretariat:view'))return; if(isMssqlMode())return sendJson(res,200,mssqlArray(`SELECT cr.*, r.nr_inregistrare, r.subiect, d.denumire as dept_nume FROM secretariat.correspondence_tracking cr JOIN secretariat.registry r ON r.id=cr.registry_id JOIN core.departments d ON d.id=cr.dept_id WHERE cr.termen_raspuns < GETDATE() AND cr.status NOT IN (N'raspuns_dat') ORDER BY cr.termen_raspuns ASC FOR JSON PATH;`)); const db=readDb(); const today=new Date().toISOString().slice(0,10); sendJson(res,200,ensureDb(db).correspondenceTracking.filter(x=>x.termen_raspuns<today&&x.status!=='raspuns_dat').map(x=>({...x,dept_nume:deptName(db,x.dept_id)})))}catch(e){next(e)}})
