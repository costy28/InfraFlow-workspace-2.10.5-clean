...
CREATE TABLE environment.autorizatii (
...
CREATE TABLE environment.coduri_deseuri (
  id INT IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE environment.deseuri (
  id INT IDENTITY(1,1) PRIMARY KEY,
  an INT NOT NULL,
...
CREATE TABLE environment.deseuri_municipale (
  id INT IDENTITY(1,1) PRIMARY KEY,
  an INT NOT NULL,
  luna TINYINT NULL,
  cod_deseu NVARCHAR(20) NOT NULL,
...
CREATE TABLE environment.emisii (
  id INT IDENTITY(1,1) PRIMARY KEY,
  an INT NOT NULL,
...
CREATE TABLE environment.monitorizare (
  id INT IDENTITY(1,1) PRIMARY KEY,
  data DATE NOT NULL,
...
CREATE TABLE environment.incidente (
  id INT IDENTITY(1,1) PRIMARY KEY,
  uuid UNIQUEIDENTIFIER NOT NULL DEFAULT NEWID(),
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_environment_deseuri_an_cod' AND object_id = OBJECT_ID(N'environment.deseuri'))
  CREATE INDEX IX_environment_deseuri_an_cod ON environment.deseuri(an, cod_deseu);
GO
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_environment_monitorizare_data' AND object_id = OBJECT_ID(N'environment.monitorizare'))
  CREATE INDEX IX_environment_monitorizare_data ON environment.monitorizare(data);
GO
