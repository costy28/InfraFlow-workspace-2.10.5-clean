export default function Input({ label, error, helperText, className = '', id, ...props }) {
  ...
  return (
    <label className="grid gap-1 text-sm font-medium text-slate-700" htmlFor={inputId}>
      ...
      <input
        id={inputId}
        className={`h-[var(--control-height)] rounded-[var(--radius-control)] border px-[var(--control-px)] text-sm outline-none transition placeholder:text-slate-400 focus:border-primary-500 focus:ring-2 focus:ring-primary-100 ${
