...
export function OperatiuniContabile() {
  ...
  function load() {
    ...
    ]).then(([statusRes, stockRes, annualRes]) => {
      ...
      <Table headers={['Inventar', 'Denumire', 'Valoare', 'Amortizat', 'Valoare netă', 'Durată', 'Acțiuni']}>
        {(status.fixed_assets || []).map(row => (
          <tr key={row.uuid}>
            ...
            <td className="px-3 py-2"><DropdownMenu align="right" label="Acțiuni" items={[{ label: 'Scoate din evidență', onClick: () => cancelAsset(row), danger: true }]} /></td>
          </tr>
        ))}
      </Table>
