  ...
  return (
    <AccountingShell active="audit-fiscal" title="Audit fiscal" subtitle="Acceptanta lunara, declaratii si SAF-T intr-un singur circuit." actions={<DropdownMenu label="Actiuni" align="right" items={[
      ...
      <Table headers={['Generat', 'Schema', 'Status', 'Conturi', 'Note', 'Facturi', 'Plati', 'Actiuni']}>
        {runs.map(run => <tr key={run.id}><td className="px-3 py-2">{run.created_at?.slice(0, 16).replace('T', ' ')}</td><td className="px-3 py-2">{run.schema_profile?.schema_version || '-'}</td><td className="px-3 py-2"><Badge tone={run.status === 'acceptat_validator' ? 'success' : run.status === 'respins_validator' ? 'danger' : 'warning'}>{run.status}</Badge></td><td className="px-3 py-2 text-right">{run.source_summary?.accounts || 0}</td><td className="px-3 py-2 text-right">{run.source_summary?.journals || 0}</td><td className="px-3 py-2 text-right">{(run.source_summary?.sales || 0) + (run.source_summary?.purchases || 0)}</td><td className="px-3 py-2 text-right">{run.source_summary?.payments || 0}</td><td className="px-3 py-2"><DropdownMenu label="Actiuni" items={[{ label: 'Descarca XML acceptat', onClick: () => download(`/accounting/saft/runs/${run.id}/download`, `D406_${month}.xml`), disabled: run.status !== 'acceptat_validator' }]} /></td></tr>)}
      </Table>
    </AccountingShell>
  )
}
