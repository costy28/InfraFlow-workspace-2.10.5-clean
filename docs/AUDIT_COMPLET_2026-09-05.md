...
1. **Securitate fișiere /storage** — rezolvată parțial în 2.12.530–2.12.531
   ...
   - Recomandare rămasă: migrare graduală a finding-urilor medium către endpoint-uri dedicate pe entitate, cu autorizare pe dosar/document/atașament. Auditul automat pentru detectarea expunerilor este introdus în v2.12.534, linkurile directe Fleet high din `FisaVehicul.jsx` și `MyVehicle.jsx` sunt închise în v2.12.535, referințele text din Mecanizare sunt sanitizate în v2.12.536, atașamentele Tichete folosesc endpoint dedicat fără path intern expus în v2.12.537, Secretariat folosește upload real plus download controlat în v2.12.538, iar schemele ANAF din Contabilitate sunt expuse prin profil public și download controlat în v2.12.539.
...
Backlog-ul acționabil este acum documentat în `docs/IMBUNATATIRI_PRIORITARE_COMERCIAL.md`. Linkurile directe high din FisaVehicul/MyVehicle sunt rezolvate în v2.12.535, referințele text din Mecanizare sunt sanitizate în v2.12.536, Tichete nu mai expune path intern în v2.12.537, Secretariat nu mai folosește cale locală în UI în v2.12.538, iar schemele ANAF nu mai expun path intern în API în v2.12.539. Auditul `npm run audit:file-exposure` raportează 0 finding-uri. Auditul de autentificări a început în v2.12.540: login reușit/eșuat/logout sunt vizibile în Setări → Securitate. Următorul pas tehnic recomandat este extinderea auditului pentru schimbări de roluri, permisiuni și stații noi.
