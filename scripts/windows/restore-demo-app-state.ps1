...
Assert-Admin
$sourceDb = Join-Path $SourceRoot "data\app-db.demo.json"
if (-not (Test-Path -LiteralPath $sourceDb)) {
  throw "Nu gasesc snapshot-ul demo: $sourceDb"
}
...
$backupDir = Join-Path $InstallRoot "backups\manual-demo-restore"
New-Item -ItemType Directory -Force -Path $backupDir | Out-Null
$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backupFile = Join-Path $backupDir "app_state_before_demo_restore_$stamp.json"

Write-Host "Scriu snapshot-ul demo in MSSQL app_state..." -ForegroundColor Cyan
...
const next = fs.readFileSync(sourceDb, 'utf8');
const parsed = JSON.parse(next);
if (!Array.isArray(parsed.users) || parsed.users.length < 1 || parsed.settings?.setupCompleted !== true) {
  throw new Error('Snapshot-ul demo nu pare valid.');
}
runMssqlScalar('if exists (select 1 from dbo.app_state where id=1) update dbo.app_state set data=@json, updated_at=sysdatetime() where id=1 else insert into dbo.app_state(id,data) values(1,@json); select 1;', { jsonInput: next });
console.log('OK');
"@ $backupFile $sourceDb
} finally {
  Pop-Location
...
Write-Host "Restore demo finalizat. Backup anterior: $backupFile" -ForegroundColor Green
